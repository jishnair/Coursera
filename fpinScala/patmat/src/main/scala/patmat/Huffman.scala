...
object Huffman {
  ...
  /**
    ...
    * }
    */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(identity).map(elem => (elem._1, elem._2.size)).toList
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: rest => (makeCodeTree(left, right) :: rest).sortBy(tree => weight(tree))
    case _ => trees
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def traverse(tree: CodeTree)(c: Char): List[Bit] = tree match {
      ...
      case Leaf(_, _) => List()
      case Fork(left, _, _, _) if (chars(left).contains(c)) => 0 :: traverse(left)(c)
      case Fork(_, right, _, _) => 1 :: traverse(right)(c)
